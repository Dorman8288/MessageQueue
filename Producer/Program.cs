...
ProducerConfigManager configManager = new();
...
try
{
    ...
    ProducerPluginLoader loader = new(logger);
    var ProducerTypes = loader.Load(configManager.DLLDirPath);
    ProductionUnit productionUnit = new ProductionUnit(ProducerTypes, "http://127.0.0.1:5155/", logger);
    await productionUnit.StartProduction();
